...
class ValidatorAgent(BaseAgent):
    def __init__(
        self,
        name: str = "validator",
        description: str = "Validates and verifies solutions and plans",
        agent_type: AgentType = AgentType.VALIDATOR,
    ):
        super().__init__(name, description, agent_type)
        self.chain = None
        self.memory = ConversationBufferMemory()
        self.template = """
        You are a validator agent that helps verify solutions and plans.
    ...
    async def process(self, context: ContextNode, input_text: str) -> AgentResponse:
        ...
        try:
            ...
            logger.error(f"Error in ValidatorAgent: {str(e)}")
            return AgentResponse(
